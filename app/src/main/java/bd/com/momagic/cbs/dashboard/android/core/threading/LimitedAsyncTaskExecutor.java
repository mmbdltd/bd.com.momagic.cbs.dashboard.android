...
import bd.com.momagic.cbs.dashboard.android.core.utilities.SemaphoreUtilities;
...
public class LimitedAsyncTaskExecutor {
    ...
    private final Logger logger = LoggerFactory.getLogger(LimitedAsyncTaskExecutor.class);
    @Getter
    ...
    public LimitedAsyncTaskExecutor() {
        // setting default limit to 0 by calling the other constructor...
        this(0);
    }

    public LimitedAsyncTaskExecutor(int defaultLimit) {
        this(defaultLimit, null);
    }

    public LimitedAsyncTaskExecutor(int defaultLimit, Map<String, Integer> limitMapByContext) {
        ...
        final Set<Map.Entry<String, Integer>> entrySet = limitMapByContext.entrySet();
        ...
        for (final Map.Entry<String, Integer> entry : entrySet) {
            ...
            final int sanitizedLimit = sanitizeLimit(entry.getValue());
            ...
            final EnhancedSemaphore semaphore = new EnhancedSemaphore(sanitizedLimit, false);
     ...
     */
    private EnhancedSemaphore retrieveSemaphore(String context) {
        ...
        if (defaultLimit > 0 && semaphore == null) {
            logger.info("Creating a new semaphore because no semaphore found for the given context, \"" + context + "\".");
    ...
    public int getLimit(String context) {
        if (context == null || limitMapByContext == null) { return getDefaultLimit(); }

        final Integer limit = limitMapByContext.get(context);

        if (limit == null) { return getDefaultLimit(); }
     ...
     */
    public AsyncTask<?> run(Runnable task) {
     ...
     * @return An AsyncTask object.
     */
    public AsyncTask<?> run(String context, Runnable task) {
        return run(context, () -> {
            task.run();
     ...
     * @return An AsyncTask object.
     */
    public <Type> AsyncTask<Type> run(Callable<Type> task) {
        return run(DEFAULT_CONTEXT, task);
    }
     ...
     * @return An AsyncTask object.
     */
    public <Type> AsyncTask<Type> run(String context, Callable<Type> task) {
        return AsyncTask.run(() -> {
            // retrieve semaphore by the provided task execution context...
            final Semaphore semaphore = retrieveSemaphore(context);

            // if semaphore is null, it means limit is zero...
            if (semaphore == null) {
            ...
            final boolean acquired = SemaphoreUtilities.tryAcquireSemaphore(semaphore);
    ...
    private static int sanitizeLimit(int limit) {
        // if limit is less than or equal to zero (0), we shall return zero (0).
