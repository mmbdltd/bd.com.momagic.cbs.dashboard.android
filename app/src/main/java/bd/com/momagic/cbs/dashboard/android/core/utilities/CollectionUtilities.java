...
public final class CollectionUtilities {
    ...
    public static boolean sequenceEqual(byte[] arrayA, byte[] arrayB) {
    ...
    public static <Type> boolean sequenceEqual(Type[] arrayA, Type[] arrayB) {
        // if both the arrays are not of the same size, we shall return false...
        if (arrayA.length != arrayB.length) { return false; }
    ...
    public static <Type> List<List<Type>> split(
        ...
        final int remainingItemCount = itemCount % itemCountPerBlock;
        // calculating the number of blocks required to hold the items...
        int blockCount = itemCount / itemCountPerBlock;
        ...
        for (int j = 0; j < blockCount; ++j) {
            ...
            for (int k = 0; i < itemCount && k < itemCountPerBlock; ++k) {
    ...
    public static <Type> List<Type> mergeAndDistributeEvenly(final List<List<Type>> listOfLists) {
        ...
        for (int i = 0; i < listOfLists.size(); ++i) {
            final List<Type> items = listOfLists.get(i);
        ...
        // initializing an array to hold the indices of each individual list within the list of lists...
        ...
        for (int i = 0, j = -1; i < totalElementCount; ) {
            // incrementing the index (j) of operator specific incoming SMS details lists...
            ++j;

            // if 'j' is greater than or equal to the size of the list of lists...
            if (j >= listOfLists.size()) {
                // we shall reset the index...
                j = 0;
            }

            final List<Type> elements = listOfLists.get(j);

            if (indices[j] >= elements.size()) { continue; }

            final Type element = elements.get(indices[j]);

            // adding the element to the list of all elements...
            allElements.add(element);

            // incrementing operator specific index...
            ++indices[j];
    ...
    public static <Type> int indexOf(final Type item, final Type[] items) {
        ...
        for (int i = 0; i < items.length; ++i) {
            ...
            final Type currentItem = items[i];
            ...
            // else if current item is equal to the given item,
    ...
    public static <KeyType, ValueType> Map<KeyType, ValueType> populateMap(final KeyType[] keys, final ValueType[] values) {
        ...
        for (int i = 0; i < length; ++i) {
            final KeyType key = keys[i];
            final ValueType value = values[i];
